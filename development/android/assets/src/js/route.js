...
forge.request.ajax({
			url: "http://parentglue.herokuapp.com/parents/"+ id +".json?auth_token="+ store.get("auth_token"),
			dataType: "json",
			success: showIndex
		});
		...
		function showIndex(data) {
			// Save away initial data
		var maker = new Pglue.Models.Parents(data, { parse: true });	
		forge.topbar.setTitle("Your Children");	
      
       
	var parents = new ParentView({model: maker });
	$(".row").empty();
	$(".row").append(parent.render().el);
}	
